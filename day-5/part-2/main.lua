...
local function swap_numbers(rules, update)
	local changed = false

	for _, number in ipairs(update) do
		for _, rule in ipairs(rules) do
			if rule[1] == number then
				local index1 = find_number_in_update(update, rule[1])
				local index2 = find_number_in_update(update, rule[2])

				-- Perform a swap if the order is invalid
				if index1 > index2 and index2 ~= -1 then
					-- Swap numbers
	...
	-- Recheck after all potential swaps
	if changed and not check_update(rules, update) then
		return swap_numbers(rules, update)
	end
...
local function main()
	...
	for i, update in ipairs(updates) do
		...
		if valid then
			...
			updates[i] = swap_numbers(rules, update)
			local middle = find_middle_number(update)
			sum = sum + middle
			print("Update " .. i .. " is invalid")
			print("Update " .. i .. " is now: " .. table.concat(updates[i], ", "))
		end
	end
