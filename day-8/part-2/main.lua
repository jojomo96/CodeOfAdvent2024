...
local function is_in_bound(x, y , width, height)
			...
			if i ~= j then
				...
				if pos1 and pos2 then  -- Validate positions exist
					...
					if delta_x and delta_y then  -- Validate deltas exist
						...
						while is_in_bound(new_x1, new_y1, width, height) do
							antinodeList:add(new_x1, new_y1)
							new_x1 = new_x1 + delta_x
							new_y1 = new_y1 + delta_y
						end

						local new_x2 = pos2[1] - delta_x
						local new_y2 = pos2[2] - delta_y
						while is_in_bound(new_x2, new_y2, width, height) do
							antinodeList:add(new_x2, new_y2)
							new_x2 = new_x2 - delta_x
							new_y2 = new_y2 - delta_y
						end
					end
				end
			end
